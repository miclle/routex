...
Personal key creation, confirmation, updates, revocation, and rotation lock the active owner before any key row. Account disable uses the same owner-first order before revoking keys, so an issuance waiting behind disable must return unauthorized. Grant replacement locks requested owners in stable ID order before the model, preserving that order during foreign-key checks. Creating a pending rotation leaves the original usable until confirmation; confirmation revokes the original before publishing the replacement.
