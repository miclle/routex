...
The egress workspace uses a compact measured-status toolbar, a six-column endpoint table, add/edit configuration dialogs, and a diagnostics dialog. Status and duration describe the last actual check; the page does not invent online state, averages, or recent timestamps. Diagnostics preserve separate transport and API results, each measured stage, skipped/not-applicable states, optional timings, and sanitized codes. An HTTP authentication rejection can establish reachable transport without implying API success.

Connection configuration keeps platform-default, explicit-direct, and named-egress selections distinct. A null platform default means direct; explicit direct never follows later platform-default changes. Provider connection creation and editing submit native mode/ID fields with the current revision. Disabled or unavailable named endpoints are never presented as automatic direct fallback.

Egress credentials use explicit keep/replace/remove actions. Stored credentials are represented only by `auth_configured`; new username/password values remain in the dialog, are cleared on dismissal, and never enter mutation caches or browser storage. Request errors retain a status only, without credential-bearing Axios configuration. New endpoints, transport/authentication edits, and re-enabling require a fresh successful draft transport test; the backend independently repeats validation on save. Name-only edits retain authentication without a test. Revision conflicts preserve the draft but block retry until an explicit reload/review, and changed connection data must be retested. Uncertain publication errors require reconciliation rather than a success notice.

Read, write, test, and provider-connection permissions remain separate. Provider selectors use the bounded authorized egress-options endpoint rather than requiring global egress-read permission. Focused tests cover permission/no-fetch boundaries, default/direct/named payloads, CSRF and revisions, explicit authentication actions, test invalidation, conflict review, duplicate dispatch, actual diagnostic stages, status-only errors, and English/Chinese draft preservation. Controlled client tests do not replace real proxy or database acceptance.
