...
| `protocol`                           | Currently `openai_chat`.                                                                                                                                                                                     |
...
This slice intentionally has no Team attribution, provider totals, CSV export, forecast, background rollup, or dashboard UI. Existing facts have connection and provider-model snapshots but no immutable provider ID. Joining the current catalog to manufacture historical provider totals would change old attribution, so that dimension requires a later schema extension. Large installations will need indexed rollups or another explicitly complete aggregation path instead of increasing these synchronous bounds without evaluation.
