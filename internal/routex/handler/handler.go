...
func (ctrl *Ctrl) RegisterRoutes(r *fox.Engine) {
	...
	identity.GET("/site", ctrl.SiteSettings)
	identity.GET("/announcements", ctrl.requireSession, ctrl.ActiveAnnouncements)
	...
	identity.DELETE("/account/mfa/enrollment", sameOrigin, ctrl.requireSession, requireCSRF, ctrl.CancelMFAEnrollment)
	identity.POST("/account/mfa/enable", sameOrigin, ctrl.requireSession, requireCSRF, jsonAuthRequest, ctrl.EnableMFA)
	identity.POST("/account/mfa/recovery-codes", sameOrigin, ctrl.requireSession, requireCSRF, jsonAuthRequest, ctrl.RegenerateMFARecoveryCodes)
	identity.POST("/account/mfa/disable", sameOrigin, ctrl.requireSession, requireCSRF, jsonAuthRequest, ctrl.DisableMFA)
	...
	admin.GET("/storage", ctrl.RequirePermission("storage.read"), ctrl.StorageSettings)
	admin.PUT("/storage", sameOrigin, requireCSRF, jsonManagementRequest, ctrl.RequirePermission("storage.write"), ctrl.WriteStorageSettings)
	admin.POST("/storage/test", sameOrigin, requireCSRF, jsonManagementRequest, ctrl.RequirePermission("storage.test"), ctrl.TestStorage)
	admin.POST("/storage/rollback", sameOrigin, requireCSRF, jsonManagementRequest, ctrl.RequirePermission("storage.write"), ctrl.RollbackStorage)
	...
	admin.PATCH("/egresses/:egress_id", sameOrigin, requireCSRF, jsonManagementRequest, ctrl.RequirePermission("egress.write"), ctrl.UpdateEgress)
	admin.POST("/egresses/test", sameOrigin, requireCSRF, jsonManagementRequest, ctrl.RequirePermission("egress.test"), ctrl.TestEgressDraft)
	admin.POST("/egresses/:egress_id/test", sameOrigin, requireCSRF, jsonManagementRequest, ctrl.RequirePermission("egress.test"), ctrl.TestEgress)
	admin.GET("/egress-default", ctrl.RequirePermission("egress.read"), ctrl.EgressDefault)
	admin.PUT("/egress-default", sameOrigin, requireCSRF, jsonManagementRequest, ctrl.RequirePermission("egress.write"), ctrl.SetEgressDefault)
	admin.GET("/egress-options", ctrl.RequirePermission("providers.read"), ctrl.EgressOptions)
	...
	admin.PATCH("/announcements/:announcement_id", ctrl.RequirePermission("announcements.write"), sameOrigin, requireCSRF, jsonManagementRequest, ctrl.UpdateAnnouncement)
	admin.POST("/announcements/:announcement_id/close", ctrl.RequirePermission("announcements.write"), sameOrigin, requireCSRF, jsonManagementRequest, ctrl.CloseAnnouncement)
