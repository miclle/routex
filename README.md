...
The active implementation goal covers the complete capability and acceptance inventory in [the implementation index](docs/IMPLEMENTATION.md). Current catalog and personal Key contracts are documented in [CATALOG](docs/CATALOG.md) and [KEYS](docs/KEYS.md). The [price catalogue and exact text quote API](docs/PRICING.md) provide price and currency administration. [Immutable gateway text assessments](docs/METERING.md) preserve each call's price basis; quota reservations and deductions remain separate work. Configure [encrypted credential storage and upstream network policy](docs/SECRET_STORAGE.md) before adding provider credentials.

Gateway and delivery contracts: [Chat inference](docs/GATEWAY.md), [native Responses](docs/RESPONSES.md), [runtime publication](docs/RUNTIME.md), [durable call records](docs/CALLS.md), [usage queries](docs/USAGE.md), [governance](docs/GOVERNANCE.md), [Teams and Projects](docs/RESOURCES.md), [Project Keys](docs/PROJECT_KEYS.md), [Project model requests](docs/PROJECT_REQUESTS.md), [offboarding](docs/OFFBOARDING.md), and [account security](docs/ACCOUNT.md), and [two-step verification](docs/MFA.md). Preserve the encryption root key and the configured local call-journal file across restarts. The journal requires persistent writable storage; production remains one process, with external integration and performance acceptance still open.
