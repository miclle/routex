...
function sse(text: string) {
  ...
  return new Response(
    new ReadableStream({
      start(stream) {
        for (let i = 0; i < data.length; i += 3) stream.enqueue(data.slice(i, i + 3))
        stream.close()
      },
    }),
...
describe('native playground client', () => {
  it('loads models with only the typed bearer Key and parses a normal response', async () => {
    const fetch = vi
      .fn()
      .mockResolvedValueOnce(Response.json({ data: [{ id: 'public-model' }] }))
      .mockResolvedValueOnce(
        Response.json(
          {
            choices: [{ index: 0, message: { content: 'Hello back' }, finish_reason: 'stop' }],
            usage: { prompt_tokens: 2, completion_tokens: 3, total_tokens: 5 },
          },
          { headers: { 'X-Request-ID': 'req_normal' } },
        ),
      )
  ...
  it('retains partial output and the request ID when a native error arrives mid-stream', async () => {
    ...
    await expect(
      runChat(key, { ...request, stream: true }, controller().signal, (result) => {
        latest = result
      }),
    ).rejects.toMatchObject({ message: 'Upstream unavailable', requestId: 'req_1' })
    expect(latest?.text).toBe('partial')
  })
  it('does not treat a truncated response as successfully completed', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(sse('data: {"choices":[{"delta":{"content":"partial"}}]}\n\n')),
    )
    await expect(
      runChat(key, { ...request, stream: true }, controller().signal, vi.fn()),
    ).rejects.toThrow('The stream ended early')
  })
  it('rejects malformed events and unexpected content types', async () => {
    const fetch = vi
      .fn()
      .mockResolvedValueOnce(sse('data: not-json\n\n'))
    ...
    await expect(
      runChat(key, { ...request, stream: true }, controller().signal, vi.fn()),
    ).rejects.toThrow('valid streaming response')
  })
      ...
      vi
        ...
        .mockResolvedValue(
          Response.json(
            ...
            { status: 401, headers: { 'X-Request-ID': 'req_error' } },
          ),
        ),
    )
    await expect(runChat(key, request, controller().signal, vi.fn())).rejects.toEqual(
      new GatewayError('Invalid Key [REDACTED]', 'req_error', 401),
    )
  })
