...
it('exposes switch state and submits only checked values', async () => {
  ...
  try {
    await act(async () => root.render(<form><Switch name="enabled" aria-label="Registration" /></form>))
    ...
    await act(async () => root.render(<form><Switch name="enabled" aria-label="Registration" disabled /></form>))
    await act(async () => control.click())
    expect(control.getAttribute('aria-checked')).toBe('true')
