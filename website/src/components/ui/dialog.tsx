...
export function Dialog({ open, onOpenChange, title, description, children, busy = false }: { open: boolean; onOpenChange: (open: boolean) => void; title: string; description: string; children: ReactNode; busy?: boolean }) {
  return <BaseDialog.Root open={open} onOpenChange={(value) => { if (!busy) onOpenChange(value) }}>
    <BaseDialog.Portal><BaseDialog.Backdrop className="fixed inset-0 z-50 bg-black/40" /><BaseDialog.Popup className="fixed top-1/2 left-1/2 z-50 max-h-[90dvh] w-[calc(100%-2rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 overflow-auto rounded-xl border bg-background p-6 shadow-xl outline-none">
