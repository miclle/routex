        ...
        >
          <header className="flex min-h-16 items-center gap-3 border-b px-6">
            <Button
              variant="ghost"
              size="icon"
              aria-label={t('close_6c14b')}
              disabled={busy}
              onClick={() => onOpenChange(false)}
            >
              <X className="size-4" />
            </Button>
            <BaseDialog.Title className="font-semibold">{title}</BaseDialog.Title>
          </header>
          <div className={`min-h-0 flex-1 overflow-auto ${bodyClassName}`}>
            {description && (
              <BaseDialog.Description className="mb-6 text-sm text-muted-foreground">
                {description}
