...
export function Page({ title, description, action, children }: { title: string; description: string; action?: ReactNode; children?: ReactNode }) {
  return <section className="mx-auto max-w-6xl space-y-6 px-6 py-10"><header className="flex flex-wrap items-start justify-between gap-4"><div className="space-y-2"><h1 className="text-2xl font-semibold">{title}</h1><p className="max-w-2xl text-sm leading-6 text-muted-foreground">{description}</p></div>{action}</header>{children}</section>
}
...
export function SaveButton({ pending, children = '保存' }: { pending: boolean; children?: ReactNode }) { return <Button type="submit" disabled={pending}>{pending ? '正在提交…' : children}</Button> }
