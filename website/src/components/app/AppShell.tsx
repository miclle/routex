...
const adminNav = [
  { to: '/admin/members', label: '成员', icon: UserRound, permission: 'members.read', group: '成员与访问' },
  { to: '/admin/roles', label: '角色与权限', icon: ShieldCheck, permission: 'roles.read', group: '成员与访问' },
  { to: '/admin/models', label: '模型', icon: Bot, permission: 'models.read_all', group: '服务接入' },
  { to: '/admin/providers', label: '供应商', icon: Cloud, permission: 'providers.read', group: '服务接入' },
  { to: '/admin/calls', label: '全平台调用', icon: History, permission: 'calls.read_all', group: '运营监控' },
  { to: '/admin/auth', label: '身份认证', icon: Settings, permission: 'registration.write', group: '系统管理' },
]
...
export default function AppShell() {
  ...
  const title = [...accountNav, ...memberNav, ...adminNav].find((item) => item.to === pathname)?.label ?? (pathname === '/admin/models/new' ? '添加模型' : pathname.startsWith('/admin/providers/') ? '供应商详情' : pathname.startsWith('/admin/models/') ? '模型详情' : pathname.startsWith('/admin/members/') ? '成员详情' : 'RouteX')
