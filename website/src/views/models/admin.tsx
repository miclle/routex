...
export default function AdminModelsPage() { return <PermissionGate permission="models.read_all"><AdminModels /></PermissionGate> }
function AdminModels() {
  const { data: session } = useSession()
  const cache = useQueryClient()
  const access = usePermissions()
  const models = useQuery({ queryKey: ['admin', 'models'], queryFn: listAdminModels })
  const providers = useQuery({ queryKey: ['admin', 'providers'], queryFn: listProviders, enabled: access.can('providers.read') })
  const grantees = useQuery({ queryKey: ['admin', 'model-grantees'], queryFn: listGrantees, enabled: access.can('models.write') })
