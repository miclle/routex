...
export default function KeysPage() {
  ...
  return <Page title="我的 API Key" description="Key 仅用于你的个人调用。密钥只展示一次，确认保存后才启用；模型范围不会超出你的有效授权。">
    ...
    <Table aria-label="API Key 列表"><thead><tr><th>名称</th><th>API Key</th><th>交付方式</th><th>模型范围</th><th>状态</th><th>有效期</th><th>操作</th></tr></thead><tbody>{keys.data?.filter((key) => statusFilter === 'all' || key.status === statusFilter).map((key) => <tr key={key.id}><td>{key.name}</td><td className="font-mono">{key.prefix}…</td><td><Badge variant="outline">浏览器一次性显示</Badge></td><td><Button variant="ghost" size="sm" onClick={() => setViewing(key)}>{key.model_ids.length} 个模型</Button></td><td><Badge variant="outline">{statuses[key.status]}</Badge></td><td className="whitespace-nowrap">{key.expires_at ? new Date(key.expires_at).toLocaleString() : '无固定期限'}</td><td><div className="flex gap-1">{(key.status === 'active' || key.status === 'disabled') && <><Button size="sm" variant="ghost" disabled={update.isPending} onClick={() => { update.reset(); setAction({ kind: 'rename', key }) }}>重命名</Button><Button size="sm" variant="ghost" disabled={update.isPending} onClick={() => update.mutate({ id: key.id, method: 'patch', data: { enabled: key.status !== 'active' } })}>{key.status === 'active' ? '停用' : '启用'}</Button><Button size="sm" variant="ghost" disabled={create.isPending} onClick={() => { create.reset(); finish.reset(); create.mutate(key) }}>轮换</Button></>}{key.status !== 'revoked' && <Button size="sm" variant="ghost" onClick={() => { update.reset(); setAction({ kind: 'revoke', key }) }}>撤销</Button>}</div></td></tr>)}</tbody></Table>
    ...
    <Dialog width={560} open={!!delivery} onOpenChange={(open) => { if (!open && !finish.isPending) finish.mutate(false) }} busy={finish.isPending} title="保存你的密钥" description="此密钥仅展示一次。确认已保存后才可使用；关闭或取消会撤销这次创建的 Key。轮换时旧 Key 在确认成功后才撤销。">
