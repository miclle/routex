...
beforeEach(async () => {
  ...
  active = true
  session = {
    user: { id: 'usr_1', name: 'User', email: 'user@example.com', role: 'member' },
    csrf_token: 'old-csrf',
  }
  sessions = [
    {
  ...
  client.defaults.adapter = async (config) => {
    ...
    const response = {
      config,
      status: failures[route] || (route === 'get /auth/session' && !active ? 401 : 200),
      statusText: '',
    ...
    if (route === 'post /account/password') {
      session.csrf_token = 'new-csrf'
      response.data = structuredClone(session)
      sessions = [{ ...sessions[0], id: 'ses_new' }]
    }
    if (config.method === 'delete' && config.url?.endsWith('/ses_current')) active = false
    if (config.method === 'delete')
      sessions = sessions.filter((item) => !config.url!.endsWith(item.id))
    return response
  }
})
afterEach(async () => {
...
async function render(security = true) {
  await act(async () => {
    root.render(
      <QueryClientProvider client={cache}>
        <MemoryRouter initialEntries={['/account']}>
          <Routes>
            <Route path="/account" element={<AccountPage security={security} />} />
  ...
  await until(() =>
    expect(container.textContent).toContain(security ? 'ses_other' : 'Basic information'),
  )
}
...
async function click(text: string) {
  await act(async () => {
    ;[...document.querySelectorAll('button')].find((item) => item.textContent === text)!.click()
  })
}

describe('account settings', () => {
  it('updates existing validation and labels when the language changes', async () => {
    await render()
    await click('Change password')
    await fill('new_password', 'short')
    ...
    expect(document.querySelector('[role="alert"]')?.textContent).toBe(
      '新密码须为 12–72 个 UTF-8 字节。',
    )
    expect(document.querySelector('form[aria-label="修改密码"]')).not.toBeNull()
  ...
  it('updates the profile using CSRF and refreshes the session user', async () => {
    ...
    await until(() => expect(container.textContent).toContain('Profile updated'))
  ...
  it('validates confirmation, rotates session state, clears password inputs and mutation data', async () => {
    ...
    await submit('Change password')
    await until(() => expect(container.textContent).toContain('Password updated'))
    ...
    await until(() =>
      ...
      ).not.toContain('new-password-value'),
    )
  ...
  it('keeps incorrect-current-password errors recoverable without logging out', async () => {
    ...
    expect(container.textContent).toContain('Security settings')
  ...
  it('revokes another session after confirmation and preserves the current session', async () => {
    ...
    await act(async () => {
      ;[...container.querySelectorAll('button')]
        .filter((item) => item.textContent === 'Revoke session')[1]
        .click()
    })
